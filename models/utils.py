...
def get_args():
    ...
    parser.add_argument("--model_name", default='cnn',
                        help="Which model to use for the text part, options are: 'avg_we' which averages the word embeddings, 'transformer', uses a 3 layer transformer, 'cnn' uses cnn for the model this is the same as state-of-the art model, 'BioBert' uses bert based models this achieves the best results")
    ...
    parser.add_argument("--CPhasW_Emb", default="0")
    ...
    parser.add_argument('--notes_aggeregate', help='How to aggeregate the numerous notes for a patient, options are: 1. Mean: Take the average of text embeddings from bert based model 2. Flat: adds all the notes of the patient as separate notes 3. Concat: concatenate all the notes of patient together 4. TimeAttn: use time attention for combining the note embeddings 5. Attn: use simple Attention', default='Mean')
    parser.add_argument('--notes_order', help='Should we get notes from beginning of the admission time or from end of it, options are: 1. First: pick first notes 2. Last: pick last notes', default='Last')
    ...
    parser.add_argument('--TSModel', help='Model to use for time series part of data, the options are: 1. LSTM: LSTM model 2. BiLSTM: Bidirectional LSTM model', default='LSTM')
    parser.add_argument('--MaxLen', help='maximum length of text to use', type=int, default=128)
    parser.add_argument('--TS_aggeregate', help='How to aggeregate the output of time series part, options are: 1.Last: Only use the last hidden state of LSTM 2. Mean: Take the average of the LSTM outputs 3. Attention: Use attention on output', default='Last')
    parser.add_argument('--Seed', help='Seed to use for both torch and np.random', type=int)
    parser.add_argument('--AttnType', help='If using Attn option for TS_aggeregate or notes_aggeregate this determines what type of attention to use options are: 1. General 2. dot', type=str, default='general')
    parser.add_argument('--NoteSplit', help='How to split notes to max model size options are truncate, concat_split, note_split', default='truncate', type=str)
